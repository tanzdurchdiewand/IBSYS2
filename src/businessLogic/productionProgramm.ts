...
export function initializeProductionProgramm(
  XML: GameData
): ProductionProgramm {
  const ChildrenSalesOrder: SalesOrder = {
    salesWish: Number(XML?.results.forecast.p1) ?? 0,
    productionWish: Number(XML?.results.forecast.p1) ?? 0,
  };

  const WommenSalesOrder: SalesOrder = {
    salesWish: Number(XML?.results.forecast.p2) ?? 0,
    productionWish: Number(XML?.results.forecast.p2) ?? 0,
  };

  const ManSalesOrder: SalesOrder = {
    salesWish: Number(XML?.results.forecast.p3) ?? 0,
    productionWish: Number(XML?.results.forecast.p3) ?? 0,
  };

  const period = Number(XML?.results.period) ?? 0;
