...
import { setProductionProgramm, updateForecastProductionProgramm, updateProductionProgramm } from "../redux/slices/inputProductionProgramm";
...
export const useProductionProgramm = () => {
    ...
    useEffect(() => {
        ...
        if (!productionProgramm && XML) {
            dispatch(
                setProductionProgramm(initializeProductionProgramm(XML))
            );
        }
    ...
    const handleValueChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (parts.length === 3) {
            ...
            if (part in { P1: true, P2: true, P3: true, directSell: true }) {
                dispatch(updateProductionProgramm({
    ...
    const handleForecastChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, bikeType: string, periodIndex: number, field: string) => {
        const { value } = event.target;
        const type = bikeType === "P1" ? "P1" :
            (bikeType === "P2" ? "P2" :
                (bikeType === "P3" ? "P3" : "P1"));
        ...
        dispatch(
            updateForecastProductionProgramm({
                ...
                [bikeType]: {
                    ...
                    forecast: productionProgramm?.[type].forecast.map((forecast: { salesOrder: any; }, index: number) => {
